...
; Instructions
;-------------------------------------------------
	AND R1, R1, #0					; This part initializes values in R1, R2, and R3 to #0
	AND R2, R2, #0
	AND R3, R3, #0
	ADD R3, R3, #2					; Adds #2 to the capacity since we are only putting in single digits for input
	
	LD R1, STACK					; Loads the address of where the stack is in memory into R1 and R2
	LD R2, STACK
	
	LEA R0, PROMPT_NUM1				; Prompts the user to enter the first number
	PUTS
	
	GETC							; Gets character from user
	OUT
	
	LD R4, CHAR_OFFSET				; Loads the character offset into R4, and subtracts it from the user input to get
	ADD R0, R0, R4					;   a number in binary.
	
	LD R4, SUB_STACK_PUSH			; Jumps to subroutine SUB_STACK_PUSH to load the binary number into the stack.
	JSRR R4
	
	LEA R0, PROMPT_NUM2				; Prompts the user to input the second number
	PUTS
	...
	LD R4, SUB_STACK_PUSH			; Jumps to subroutine SUB_STACK_PUSH to load binary number onto the stack
	JSRR R4	
	
	LD R4, SUB_RPN_MULTIPLY_3600	; Jumps to subroutine SUB_RPN_MULTIPLY_3600	to multiply last two items in the stack
	JSRR R4
	
	AND R1, R0, #-1
	LD R4, SUB_PRINT_CHARS_4100		; Prints out the product of the last two items into the stack as characters to console
	JSRR R4
...
; (2) Subroutine's Algorithm		
	...
	ADD R3, R3, #-1						; Subtracts one from the available capacity
	BRzp NO_OVERFLOW_3200				; If capacity is #0, the stack has overflown
		LEA R0, OVERFLOW_PROMPT_3200	; Prints overflow message to user
		PUTS
		BRnzp OVERFLOWED_3200			; Jumps to after RET to end program

NO_OVERFLOW_3200:
		
	STR R0, R2, #0						; Stores new value at top of stack
	ADD R2, R2, #1						; Increments to the new top of the stack
...
; (2) Subroutine's Algorithm	
	...
	NOT R5, R2							; Inverts the address in R2 to a negative value
	ADD R5, R5, #1						; This basically subtracts the value in R1 with R2
	ADD R5, R5, R1						; If this is #0, we know there is an underflow of the stack 
	BRnp NO_UNDERFLOW_3400
		LEA R0, UNDERFLOW_PROMPT		; Alerts the user of an underflow in the stack
		PUTS
		BRnzp UNDERFLOWED_3400			; Jumps to after RET to end the program at HALT
		
NO_UNDERFLOW_3400:

	ADD R3, R3, #1						; Increments the capacity
	ADD R2, R2, #-1						; Decrements the value of address for the top of the stack
...
; (2) Subroutine's Algorithm		
	...
	LD R6, SUB_STACK_POP_3400			; Loads the address of the subroutine into R6
	
	JSRR R6								; Jumps to subroutine SUB_STACK_POP_4300 to point R2 to the last value in the stack
	LDR R4, R2, #0						; Loads the last value of the stack into R4
	
	JSRR R6								; Jumps to subroutine SUB_STACK_POP_4300 to point R2 to the last value in the stack
	LDR R5, R2, #0						; Loads the last value of the stack into R5
	
	LD R6, SUB_MULTIPLY_3800			; Loads the address of the subroutine into R6
	JSRR R6								; Jumps to subroutine to multiply values in R4 and R5 together
	
	LD R6, SUB_STACK_PUSH_3200			; Loads the subroutine into R6
	JSRR R6								; Loads the product of the values, which is in R0, to the top of the stack
